...
class AllTasksFragment : Fragment() {
    ...
    override fun onResume() {
        ...
        this.mViewModel.findAll()
    }

    private fun observe() {
        this.mViewModel.findAll.observe(viewLifecycleOwner, Observer {
            if (it != null) {
                this.mAdapter.updateListener(it)
//                this.mAdapter.notifyDataSetChanged()
            }
        })

        this.mViewModel.validation.observe(viewLifecycleOwner, Observer {
            if (it.isSuccessful()) {
                this.showToast(getString(R.string.task_removed))
            } else {
